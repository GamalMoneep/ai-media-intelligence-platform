...
from src.main.podcast_fetcher import PodcastFetcher
...
class PodcastService:
    ...
    def _chunk_text(self, text: str, max_chunk_size: int) -> List[str]:
        ...
        logger.info(
            f"Total tokens: {total_tokens}, Optimal chunks: {optimal_chunks}, Target tokens per chunk: {optimal_token_size}")
        ...
        try:
            ...
            for chunk_index, chunk in enumerate(chunks, 1):
                ...
                try:
                    ...
                    logger.debug(
                        f"Raw API response for chunk {chunk_index}: {summary_text[:500]}...")  # Log first 500 chars
...
{{
    ...
    def refresh_episodes(self) -> Dict:
        ...
        try:
                ...
                if sub['type'] == 'podcast':
                    ...
                    try:
                        ydl_opts = {
                            ...
                            'no_download': True,
                            'skip_download': True,
                            'playlistend': 70  # Increased to get more videos for categorization
                        }
                                ...
                                for video in channel_info['entries']:
                                    if video:
                                        ...
                                        if video_id:
                                            ...
                                            is_short = video.get('duration',
                                                                 0) <= 60  # Shorts are typically ≤ 60 seconds
                                            is_stream = video.get('was_live', False) or 'stream' in video.get('title',
                                                                                                              '').lower()
            ...
            print(
                f"Found {len(new_episodes['podcast'])} podcast episodes and {len(new_episodes['youtube'])} YouTube videos")
            return new_episodes
        except Exception as e:
