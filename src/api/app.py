...
from src.main.service import PodcastService

app = FastAPI(title="Quantum AI Podcast API")
...
async def get_tts(episode_id: str):
    try:
        ...
        if tts_paths.get('comprehensive_summary'):
            paths = tts_paths['comprehensive_summary'] if isinstance(tts_paths['comprehensive_summary'], list) else [
                tts_paths['comprehensive_summary']]
            for path in paths:
                audio_files.append({
        ...
        if tts_paths.get('key_insights'):
            paths = tts_paths['key_insights'] if isinstance(tts_paths['key_insights'], list) else [
                tts_paths['key_insights']]
            for path in paths:
                audio_files.append({
                    "filename": Path(path).name,
        ...
        if tts_paths.get('action_items'):
            paths = tts_paths['action_items'] if isinstance(tts_paths['action_items'], list) else [
                tts_paths['action_items']]
            for path in paths:
                audio_files.append({
                    "filename": Path(path).name,
    ...
    try:
        ...
        async def generate_summary_task():
            try:
                ...
                summary = service._generate_structured_summary(transcript_text,
                                                               target_language=language.get('language', 'en'))
                if not summary:
                    print("Failed to generate summary")
                    return
